...
class AdminController extends Controller
{
    public function dashboard()
    {
        ...
        // Analytics Data: Orders by Status
        $ordersCountByStatus = \App\Models\Order::selectRaw('status, count(*) as total')
            ->groupBy('status')
            ->pluck('total', 'status');

        $orderStatusLabels = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];
        $orderStatusData = collect($orderStatusLabels)->mapWithKeys(function ($status) use ($ordersCountByStatus) {
            return [$status => $ordersCountByStatus->get($status, 0)];
        });

        // Analytics Data: User Registrations (Last 7 Days)
        $userRegData = \App\Models\User::selectRaw('DATE(created_at) as date, count(*) as total')
            ->where('created_at', '>=', now()->subDays(7))
            ->groupBy('date')
            ->pluck('total', 'date');
        $userGrowthData = $dates->merge($userRegData);

        // Analytics Data: Top 5 Selling Products
        $topProducts = \App\Models\OrderItem::selectRaw('product_name, SUM(quantity) as total_qty')
        ...
        return view('admin.dashboard', compact(
            ...
            'productsByCategory', 'orderStatusData', 'userGrowthData', 'topProducts'
        ));
    }
