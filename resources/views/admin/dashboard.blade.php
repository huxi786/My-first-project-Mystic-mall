...
<div class="row mb-5">
    ...
    <div class="col-lg-4 mb-4 mb-lg-0">
        <div class="card card-custom h-100 shadow-sm border-0">
            <div class="card-body p-4">
                <h5 class="fw-bold text-dark mb-4"><i class="fas fa-chart-pie me-2 text-warning"></i>Orders Distribution</h5>
                <div style="position: relative; height: 300px; width: 100%;">
    ...
    <div class="col-lg-8">
        <div class="card card-custom h-100 shadow-sm border-0">
            <div class="card-body p-4">
                <h5 class="fw-bold text-dark mb-4"><i class="fas fa-chart-bar me-2 text-primary"></i>User Growth (Last 7 Days)</h5>
                <div style="position: relative; height: 300px; width: 100%;">
    ...
    <div class="col-lg-4 mb-5">
        <div class="card card-custom h-100 shadow-sm border-0">
            <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                <h5 class="fw-bold text-dark mb-0"><i class="fas fa-medal me-2 text-warning"></i>Top Selling Items</h5>
            </div>
            <div class="card-body p-4">
                <div class="list-group list-group-flush">
                    @forelse($topProducts as $index => $item)
                    <div class="list-group-item border-0 px-0 py-3 d-flex align-items-center">
                        <div class="avatar-circle me-3" style="background-color: {{ $index == 0 ? '#ffc800' : ($index == 1 ? '#e0e0e0' : ($index == 2 ? '#cd7f32' : 'rgba(46, 2, 73, 0.1)')) }}; color: {{ $index < 3 ? '#fff' : '#2e0249' }};">
                            {{ $index + 1 }}
                        </div>
    ...
    <div class="col-lg-8 mb-5">
        <div class="card card-custom shadow-sm border-0 h-100">
            <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                <h5 class="fw-bold text-dark mb-0"><i class="fas fa-history me-2 text-primary"></i>Recent Orders</h5>
                                            ...
                                            <tr>
                                                <td class="ps-4">
                                                    <div class="d-flex align-items-center">
                                                        <img src="{{ Str::startsWith($product->image, 'http') ? $product->image : asset('uploads/' . $product->image) }}" alt="Product" class="rounded-3 shadow-sm me-3" width="48" height="48" style="object-fit: cover;">
                                                        <div>
...
<script>
    // Ensure we destroy old charts if this script runs multiple times (e.g. via Livewire or Turbolinks)
    if (window.salesChartInstance) { window.salesChartInstance.destroy(); }
    if (window.orderStatusChartInstance) { window.orderStatusChartInstance.destroy(); }
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    ticks: { color: '#666', font: { family: "'Poppins', sans-serif" } }
                },
                x: {
                    grid: { display: false },
                    ticks: { color: '#666', font: { family: "'Poppins', sans-serif", size: 11 } }
                }
            }
        }
    });
    ...
    window.userGrowthChartInstance = new Chart(ctxGrowth, {
        type: 'bar',
        data: {
            labels: {!! json_encode($userGrowthData->keys()) !!},
            datasets: [{
                label: 'New Registrations',
                data: {!! json_encode($userGrowthData->values()) !!},
                backgroundColor: '#2e0249', // Dark Purple
                borderRadius: 4,
                barThickness: 20,
                hoverBackgroundColor: '#ffc800' // Gold on hover
            }]
        },
        options: {
            responsive: true,
